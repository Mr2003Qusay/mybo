...
def load_grizzly_config() -> dict:
    global active_rentals, valid_numbers_found, sms_notified_ids
...
async def handle_new_activation_link(app_bot, raw_link: str) -> str:
    ...
    if "collected_links" not in cfg:
        cfg["collected_links"] = []
        
    cfg["collected_links"].append(cleaned)
    save_grizzly_config(cfg)
    
    logger.info(f"Collected new link: {cleaned}. Total collected in current batch: {len(cfg['collected_links'])}")
    
    if len(cfg["collected_links"]) >= 10:
        links_to_send = cfg["collected_links"][:10]
        ...
        try:
            ...
            cfg["collected_links"] = cfg["collected_links"][10:]
            save_grizzly_config(cfg)
            ...
            async def validate_jio_and_manage(ph=phone, aid=act_id, prov=provider, cst=cost):
                ...
                if is_valid:
                    ...
                    if cfg_current.get("google_one_activation", True):
                        ...
                        # 2. Wait for OTP code — actively poll get_sms API + check poller
                        start_wait = time.time()
                        otp_code = None
                        r_state = active_rentals.get(aid, {})
                        end_time = r_state.get("endTime", start_wait + 900)
                        ...
                        while time.time() < end_time:
                            ...
                            # Direct get_sms check every 5 seconds (bypasses rate limiter — critical path)
                            if pva_key and prov == "pvacodes" and time.time() - last_direct_check >= 5:
                                last_direct_check = time.time()
                                try:
                        ...
                        if not otp_code:
                            await app_bot.send_message(OWNER_ID, f"⏱ No OTP received within lease duration. Expired number +{current_phone}")
                            await cancel_number(prov, aid, keys)
                            if aid in active_rentals:
                                active_rentals[aid]["status"] = "cancelled"
                                active_rentals[aid]["removedAt"] = time.time()
