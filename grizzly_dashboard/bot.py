...
async def check_jio_number(phone: str) -> bool:
    ...
    try:
        async with httpx.AsyncClient(transport=_transport, timeout=7) as client:
...
async def jio_validate_otp(phone: str, otp: str) -> dict:
    ...
    try:
        async with httpx.AsyncClient(transport=_transport, timeout=15, cookies=cookies_dict) as client:
            resp = await client.post(url, json=payload, headers=headers)
